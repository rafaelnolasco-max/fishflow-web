...
// Proxy de descarga de XML via Facturapi (no expone la API key al cliente)
...
export async function GET(
  ...
  const facturRes = await fetch(
    `https://www.facturapi.io/v2/invoices/${invoice.facturapi_id}/xml`,
    { headers: { Authorization: `Bearer ${apiKey}` } }
  )

  if (!facturRes.ok) {
    return NextResponse.json({ error: 'Error al descargar XML' }, { status: 502 })
  }
