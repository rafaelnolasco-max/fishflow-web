...
export default function FinanzasRegistro() {
  ...
  // Excepción: si viene de un link de recuperación, primero debe poner
  // contraseña nueva (evento PASSWORD_RECOVERY).
  useEffect(() => {
    let done = false;
    ...
    // Con ?code= en la URL, esperar un momento a que llegue PASSWORD_RECOVERY
    // antes de auto-entrar (si era confirmación de correo, entra igual).
  ...
  async function sendRecovery() {
    ...
    const { error } = await supabase.auth.resetPasswordForEmail(email.trim(), {
      redirectTo: `${window.location.origin}/finanzas/registro`,
    });
    setBusy(false);
    ...
    <div style={{ minHeight: "100vh", background: FF_DARK, color: TEXT, display: "flex",
      ...
      <div style={{ width: "100%", maxWidth: 400 }}>
        ...
        <div style={{ background: SURFACE, border: `1px solid ${BORDER}`, borderRadius: 16, padding: 22 }}>
          {mode === "reset" ? (
            /* ── Nueva contraseña (llegó desde link de recuperación) ── */
