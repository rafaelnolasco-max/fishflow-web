...
import {
  StatGrid, StatCard, Section as DSection, Field as DField, Chip, Modal as DModal,
...
const C = {
  bg: "#F7F9FC", white: "#FFFFFF", teal: "#2A9D8F", tealLight: "#E0F4F2",
  text: "#1A1A2E", muted: "#6B7280", border: "#E5E7EB",
  red: "#EF4444", green: "#10B981", yellow: "#F59E0B", blue: "#3B82F6", gray: "#9CA3AF",
} as const;
...
export default function ReviewsTab() {
  ...
  async function fetchAll() {
    ...
    const [{ data: st, error: e1 }, { data: rq, error: e2 }] = await Promise.all([
      supabase.from("review_settings").select("*").eq("client_id", CANE_CLIENT_ID).maybeSingle(),
      supabase.from("review_requests").select("*").eq("client_id", CANE_CLIENT_ID)
  ...
  async function importCsv() {
    ...
    for (const row of fresh) {
      const { error } = await supabase.from("review_requests").insert({
        client_id: CANE_CLIENT_ID, contact_name: row.name, contact_phone: row.phone, source: "csv",
      });
      if (!error) inserted++;
    ...
    return (
      <div style={{ ...cardStyle, padding: 14, display: "flex", flexWrap: "wrap", gap: 10, alignItems: "center" }}>
        <div style={{ minWidth: 140, flex: "1 1 160px" }}>
          <div style={{ fontSize: 14, fontWeight: 700, color: C.text }}>{r.contact_name}</div>
          <div style={{ fontSize: 12, color: C.muted }}>{r.contact_phone} · {SOURCE_LABEL[r.source]}</div>
        </div>
        <Chip label={STAGE_LABEL[r.stage]} bg={r.stage === 3 ? `${C.green}20` : `${C.blue}15`} fg={r.stage === 3 ? C.green : C.blue} />
  ...
  // ── Render ──────────────────────────────────────────────────────────────────
  if (loading) return <div style={{ textAlign: "center", padding: 60, color: C.muted }}>Cargando reseñas...</div>;
  ...
  return (
    <>
      {!settings?.review_link && (
        <div style={{
          ...
          <button onClick={() => setShowSettings(true)} style={{
            background: "none", border: "none", color: "#92400E", fontWeight: 700,
      ...
      <StatGrid>
        <StatCard theme={T} icon="⭐" label={`Reseñas logradas (meta ${settings?.review_goal ?? 25})`} value={completed.length} accent={C.green} />
        ...
        <StatCard theme={T} icon="🔗" label="Link de reseña" value={settings?.review_link ? "Listo ✓" : "Pendiente"} accent={settings?.review_link ? C.green : C.yellow} />
      </StatGrid>

      <Section
        title="Cola de reseñas"
        action={{ label: "+ Agregar paciente", onClick: () => setShowAdd(true) }}
      >
        <div style={{ display: "flex", gap: 10, marginBottom: 16, flexWrap: "wrap" }}>
          <button onClick={() => setShowCsv(true)} style={{
            background: "none", border: `1px solid ${C.border}`, borderRadius: 8,
            padding: "7px 14px", fontSize: 13, color: C.text, cursor: "pointer", fontWeight: 600,
          }}>📄 Importar CSV</button>
          <button onClick={() => setShowSettings(true)} style={{
            background: "none", border: `1px solid ${C.border}`, borderRadius: 8,
            padding: "7px 14px", fontSize: 13, color: C.text, cursor: "pointer", fontWeight: 600,
        ...
        {active.length === 0 ? (
          <Empty theme={T} msg="Sin pacientes en la cola. Agrega uno, importa un CSV o usa ⭐ desde una cita." />
        ) : (
          ...
          <div style={{ marginTop: 18, display: "flex", flexDirection: "column", gap: 8 }}>
            {finished.map(r => (
              <div key={r.id} style={{ ...cardStyle, padding: "10px 14px", display: "flex", gap: 10, alignItems: "center", opacity: .75 }}>
                <div style={{ flex: 1, fontSize: 13, color: C.text }}>{r.contact_name}</div>
                <Chip
                  label={r.status === "completed" ? "⭐ Reseña" : r.status === "negative_feedback" ? "Feedback privado" : r.status === "no_response" ? "Sin respuesta" : "Declinó"}
                  bg={r.status === "completed" ? `${C.green}20` : `${C.gray}20`}
                  fg={r.status === "completed" ? C.green : C.muted}
                />
              </div>
            ))}
          </div>
      ...
      {/* ── Modal alta manual ── */}
      {showAdd && (
        <Modal title="Agregar paciente a la cola" onClose={() => setShowAdd(false)}>
          <Field label="Nombre *">
      ...
      {/* ── Modal CSV ── */}
      {showCsv && (
        <Modal title="Importar pacientes (CSV)" onClose={() => { setShowCsv(false); setCsvRows([]); }}>
          ...
          {csvRows.length > 0 && (
            <>
              ...
              <div style={{ maxHeight: 180, overflowY: "auto", border: `1px solid ${C.border}`, borderRadius: 8, padding: 10, marginBottom: 8 }}>
                {csvRows.slice(0, 50).map((r, i) => (
                  <div key={i} style={{ fontSize: 12.5, color: C.muted, padding: "2px 0" }}>
                    {r.name} — {r.phone}
                  </div>
                ))}
                {csvRows.length > 50 && <div style={{ fontSize: 12, color: C.gray }}>…y {csvRows.length - 50} más</div>}
              </div>
            </>
          )}
        ...
        <Modal title="Configuración de reseñas" onClose={() => setShowSettings(false)} wide>
          <Field label="Google Place ID">
            ...
            <div style={{ fontSize: 11.5, color: C.muted, marginTop: 4 }}>
              Búscalo en{" "}
              <a href="https://developers.google.com/maps/documentation/places/web-service/place-id" target="_blank" rel="noreferrer" style={{ color: C.teal }}>
                Place ID Finder
              </a>{" "}con &quot;CANE Neurofeedback&quot;.
            </div>
          </Field>
          {settings?.review_link && (
            <div style={{ fontSize: 12.5, color: C.muted, marginBottom: 14, wordBreak: "break-all" }}>
              Link actual: <a href={settings.review_link} target="_blank" rel="noreferrer" style={{ color: C.teal }}>{settings.review_link}</a>
            </div>
          )}
          ...
          <div style={{ fontSize: 11.5, color: C.muted, marginBottom: 10 }}>
            Usa <b>{"{nombre}"}</b> para el nombre del paciente y <b>{"{link}"}</b> para el link de reseña.
          </div>
